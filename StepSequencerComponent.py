            ...
            if self._sequencer_clip != None:# and self._sequencer_clip.is_midi_clip:
                ...
                if self._sequencer_clip.is_midi_clip:
                    for note in self._clip_notes:
                        position = note[1] #position in beats; range == 0.x to 15.x for 4 measures in 4/4 time (equivalent to 1/4 notes)
                        bank = int(position / self._quantization / self._width) #at 1/16th resolution in 4/4 time, each bank == 1/2 measure wide
        ...
        if self.is_enabled() and self._is_active:
            if self._sequencer_clip != None and self._sequencer_clip.is_midi_clip:
                if ((value != 0) or (not is_momentary)):
                    ...
                    self._sequencer_clip.select_all_notes()
                    self._replace_notes(tuple(note_cache))



    def _replace_notes(self, notes):
        try:
            # Silence the DeprecationWarning for replace_selected_notes
            import warnings
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", DeprecationWarning)
                self._sequencer_clip.replace_selected_notes(notes)
        except:
            # Fallback if something goes wrong with warning suppression
            self._sequencer_clip.replace_selected_notes(notes)
