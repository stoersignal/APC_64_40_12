...
class CustomTransportComponent(ControlSurfaceComponent):
    ...
    def set_tempo_control(self, control, fine_control = None):
        assert ((control == None) or (isinstance(control, EncoderElement) and (control.message_map_mode() == Live.MidiMap.MapMode.absolute)))
        assert ((fine_control == None) or (isinstance(fine_control, EncoderElement) and (fine_control.message_map_mode() == Live.MidiMap.MapMode.absolute)))
        if (self._tempo_control != None):
            self._tempo_control.remove_value_listener(self._tempo_value)
